/* App élève avec import JSON + mailto */
const APSA_CONFIG = {
  "Demi-fond": {
    required: ["date", "rpe"],
    fields: [
...
function downloadText(name,text,mime="text/plain"){
  ...
  const a=document.createElement("a"); a.href=URL.createObjectURL(blob); a.download=name; a.click(); setTimeout(()=>URL.revokeObjectURL(a.href),1e3);
}
...
window.CEPS=(function(){
  ...
  function init(){
    ...
    $("#activite").value="Demi-fond"; renderDyn();
  ...
  function renderDyn(){
    const cfg=APSA_CONFIG[$("#activite").value], host=$("#dyn_fields"); host.innerHTML="";
    cfg.fields.forEach(f=>{
      const wrap=document.createElement("div"); wrap.className="field";
      const lab=document.createElement("label"); lab.textContent=f.label+(f.required?" *":""); wrap.appendChild(lab);
      let input; if(f.type==="select"){ input=document.createElement("select"); (f.options||[]).forEach(o=>{const op=document.createElement("option");op.value=o;op.textContent=o;input.appendChild(op);});}
      else if(f.type==="textarea"){ input=document.createElement("textarea"); }
      else { input=document.createElement("input"); input.type=f.type; if(f.step)input.step=f.step; if(f.min)input.min=f.min; if(f.max)input.max=f.max; if(f.type==="date") input.value=todayStr(); }
      input.id="f_"+f.key; wrap.appendChild(input); host.appendChild(wrap);
    });
  }

  function collect(){
    const cfg=APSA_CONFIG[$("#activite").value]; const o={};
    cfg.fields.forEach(f=>{ const el=document.getElementById("f_"+f.key); let v=el?el.value:""; if(f.type==="number"&&v!=="") v=Number(v); o[f.key]=v; });
    (cfg.required||[]).forEach(r=>{ if(!o[r]) throw new Error("Champ obligatoire manquant : "+r); });
    if(!o.date) o.date=todayStr(); return o;
  }

  function onSaveSeance(){
    try{
      if(!state.eleve.id||!state.trimestre){ alert("Renseignez ID + Trimestre puis Charger/Créer."); return; }
      const act=$("#activite").value, s=collect();
      ...
      const row={"Source":"CahierEPS","ID":state.eleve.id,"Nom":state.eleve.nom,"Prénom":state.eleve.prenom,"Classe":state.eleve.classe,"Sexe":state.eleve.sexe,"Trimestre":state.trimestre,"Séance #":num,"Date":s.date,"Activité":act,"Session ID":sid};
      const map={"duree_min":"Durée (min)","distance_m":"Distance (m)","fc_moy":"FC moy","commentaire":"Commentaire","rpe":"RPE","analyse":"Analyse","repos":"Position de repos respectée","mousquetonnage":"Mousquetonnage","repetition_min":"Répétition (min)","t50_1":"Temps 50m #1 (s)","t50_2":"Temps 50m #2 (s)","t50_3":"Temps 50m #3 (s)","cb_50_3":"Coups de bras (50m3)"};
  ...
  function renderTable(){
    ...
    rows.forEach(r=>{ const tr=document.createElement("tr"); headers.forEach(h=>{ const td=document.createElement("td"); td.textContent=r[h]??""; tr.appendChild(td); }); tr.addEventListener("click",()=>showQR(r)); tbody.appendChild(tr); });
  }

  function showQR(row){
    $("#qr_section").classList.remove("hidden");
    const payload=JSON.stringify(row);
    const canvas=window.QRCodeGen(payload);
    const host=$("#qr_host"); host.innerHTML=""; host.appendChild(canvas);
  ...
  function onImportJSON(ev){
    ...
    reader.onload=()=>{
      try{
        ...
        if(!obj||!obj.seances){ alert("JSON invalide"); return; }
        state.cahier.seances = Array.isArray(obj.seances)? obj.seances : [];
        saveCahier(state.eleve.id,state.trimestre,state.cahier); renderTable();
