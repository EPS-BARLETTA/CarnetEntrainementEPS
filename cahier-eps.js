/* See previous build: same logic with identity, unlimited sessions, CSV/JSON export, QR per session */
// ... (identical to prior, with small footer compatibility) ...
...
function makeSessionId({eleveId, trimestre, date, activite}) {
  return `ceps-${eleveId}-${trimestre}-${date}-${activite}`.toLowerCase().replace(/\s+/g,'_');
}
function storageKey(eleveId, trimestre){ return `cahier_eps:${eleveId}:${trimestre}`; }
function loadCahier(eleveId, trimestre){
  const raw = localStorage.getItem(storageKey(eleveId,trimestre));
  if (!raw) return {meta:{created_at:new Date().toISOString()}, seances:[]};
  try { return JSON.parse(raw);} catch(e){ return {meta:{}, seances:[]}; }
}
function saveCahier(eleveId, trimestre, data){
  data.meta = {...(data.meta||{}), updated_at:new Date().toISOString()};
  localStorage.setItem(storageKey(eleveId,trimestre), JSON.stringify(data));
}
function toCSV(rows){
...
window.CEPS = (function(){
  ...
  function init(){
    ...
    $("#activite").value = "Demi-fond";
    renderDyn();

    $("#btn_load").addEventListener("click", load);
    $("#activite").addEventListener("change", renderDyn);
    $("#btn_save_seance").addEventListener("click", saveSeance);
  ...
  function collect(){
    ...
    (cfg.required||[]).forEach(req=>{ if(!o[req]) throw new Error("Champ obligatoire manquant : "+req); });
  ...
  function saveSeance(){
    try{
      if (!state.eleve.id || !state.trimestre){ alert("Renseignez ID élève et Trimestre puis cliquez sur Charger/Créer."); return; }
      const act = $("#activite").value;
      const s = collect();
      const num = (state.cahier.seances?.length || 0) + 1;
      const sid = makeSessionId({eleveId:state.eleve.id, trimestre:state.trimestre, date:s.date, activite:act});
      const row = {
        "Source":"CahierEPS","ID":state.eleve.id,"Nom":state.eleve.nom,"Prénom":state.eleve.prenom,
        "Classe":state.eleve.classe,"Sexe":state.eleve.sexe,"Trimestre":state.trimestre,
        "Séance #":num,"Date":s.date,"Activité":act,"Session ID":sid
      };
      const map = {"duree_min":"Durée (min)","distance_m":"Distance (m)","fc_moy":"FC moy","commentaire":"Commentaire","rpe":"RPE",
        "analyse":"Analyse","repos":"Position de repos respectée","mousquetonnage":"Mousquetonnage",
        "repetition_min":"Répétition (min)","t50_1":"Temps 50m #1 (s)","t50_2":"Temps 50m #2 (s)","t50_3":"Temps 50m #3 (s)","cb_50_3":"Coups de bras (50m3)"};
      for (const [k,v] of Object.entries(s)){ row[map[k]||k] = v; }
      state.cahier.seances.push(row); saveCahier(state.eleve.id, state.trimestre, state.cahier);
      renderTable(); showQR(row); window.scrollTo({top:0,behavior:"smooth"});
    }catch(e){ alert(e.message||String(e)); }
  }

  function renderTable(){
    const rows = state.cahier.seances || [];
    document.getElementById("counter").textContent = `${rows.length} séance(s)`;
    const defaultH = ["Source","ID","Nom","Prénom","Classe","Sexe","Trimestre","Séance #","Date","Activité","Session ID"];
    const extra = new Set(); rows.forEach(r=>Object.keys(r).forEach(k=>{ if(!defaultH.includes(k)) extra.add(k); }));
    const headers = [...defaultH, ...extra];
  ...
  function showQR(row){
    ...
    const host = document.getElementById("qr_host"); host.innerHTML=""; host.appendChild(canvas);
    document.getElementById("qr_text").textContent = payload;
  }

  function exportCSV(){
    const rows = state.cahier.seances||[]; if(!rows.length){ alert("Aucune séance."); return; }
    const csv = toCSV(rows);
    downloadText(`CahierEPS_${state.eleve.id}_${state.trimestre}.csv`, csv, "text/csv");
  }
  function exportJSON(){
    const rows = state.cahier.seances||[];
    const obj = {app:"CahierEPS", v:1, eleve:state.eleve, trimestre:state.trimestre, seances:rows};
    downloadText(`CahierEPS_${state.eleve.id}_${state.trimestre}.json`, JSON.stringify(obj,null,2), "application/json");
  }

  return { init };
})();

document.addEventListener("DOMContentLoaded", ()=> CEPS.init());
